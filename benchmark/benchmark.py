...
def run_data_put(client_conn, thr_id, key_prefix, num_ios=0):
    for i in range(num_ios):
        filename = '/tmp/%s-object-%s-%d' % (key_prefix, thr_id, i)
...
def run_data_get(client_conn, thr_id, key_prefix, num_ios=0):
    ...
    for i in range(num_ios):
        filename = '%s-object-%s-%d' % (key_prefix, thr_id, i)
        with open(filename, 'rb') as file:
            ...
            if output_md5 != object_data_md5:
                logger.error("Key %s didn't match with object data", filename)

    logger.info('GET objects - count %d - time %d seconds', count, (end_time - start_time))
...
def run_data_del(client_conn, thr_id, key_prefix, num_ios=0):
    ...
    logger.info('DEL objects - count %d - time %d', count, (end_time - start_time))
