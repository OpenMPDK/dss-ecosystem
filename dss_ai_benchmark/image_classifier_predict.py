...
import sys
import os
from models.pytorch import ConvNet
...
for imagePath in imagePaths:
    # load the image, perform some preprocessing
    image = cv2.imread(imagePath)
    fileName = imagePath.strip().split('/')[-1].split('.')[0]
    copied_img = image.copy()
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    image = cv2.resize(image, tuple(config["dataset"][config["dataset"]["choice"]]["image_dimension"]))
    ...
    image = image.float()
    print(f"\n\n[INFO] Image preprocessed {image.shape} and going for predictions...")
    ...
    i = preds.argmax(dim=-1).cpu()
    label = categories[i]
    print('\nPrediction Probability distribution:')
    print(preds)
    print(preds.argsort(dim=-1).cpu()[0][-5:])
    pnt = [categories[j] for j in preds.argsort(dim=-1).cpu()[0][-5:]]
    print(pnt)
    print(f'Predicted category with highest probability: {label}')
    print('\n-' + '-' * 50 + '\n')

    # resize the original image such that it fits on our screen
    copied_img = imutils.resize(copied_img, width=600)
    (h, w) = copied_img.shape[:2]

    # draw the predicted class label on the image
    cv2.putText(copied_img, label, (250, 20), cv2.FONT_HERSHEY_SIMPLEX,
                0.65, (0, 255, 0), 2)
    print("[INFO] Label predicted and written to output image...")
