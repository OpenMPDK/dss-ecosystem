...
```
##### Build elbencho with DSS Support
To build with DSS support, build dss_client using the steps outlined in the README file of that directory
...
Do `elbencho --help-s3` to see which options are supported with DSS
Documentation for dependencies, known issues, and installation will be updated over time.
Current known issues:
- No debug logs for DSS related things in elbencho
- GDS not supported with S3
- Depending on which libverbs is installed and compiled with, elbencho may not run. Will update once further understood/fixed.
