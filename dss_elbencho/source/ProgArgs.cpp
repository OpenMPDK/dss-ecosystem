...
/*DSS*/ (ARG_DSS_LONG, bpo::bool_switch(&this->useDSS),
            "Use DSS Client library with S3. Only PUT, GET, and DEL as of now.")
/*DSS*/ (ARG_UUID_LONG, bpo::value(&this->uuid),
            "Define a UUID to give to DSS Client.")
/*DSS*/ (ARG_NUMENDPOINTS_LONG "," ARG_NUMENDPOINTS_SHORT, bpo::value(&this->numEndpoints),
            "Number of endpoints per cluster to use for DSS client.")
#endif // S3_SUPPORT
...
void ProgArgs::printHelpS3()
{
	...
	argsS3MiscDescription.add_options()
		...
		(ARG_NUMAZONES_LONG, bpo::value(&this->numaZonesStr),
			...
			"given, then worker threads are bound round-robin to the zones. "
			"(Hint: See 'lscpu' for available NUMA zones.)")
        (ARG_DSS_LONG, bpo::bool_switch(&this->useDSS),
            "Use DSS Client library with S3. Only PUT, GET, and DEL as of now.")
        (ARG_UUID_LONG, bpo::value(&this->uuid),
            "Assign a UUID to this DSS client instance")
        (ARG_NUMENDPOINTS_LONG "," ARG_NUMENDPOINTS_SHORT, bpo::value(&this->numEndpoints),
            "Number of endpoints per cluster DSS Client should use. Default: 256")
	;
...
void ProgArgs::setFromPropertyTreeForService(bpt::ptree& tree)
{
    ...
    useDSS = tree.get<bool>(ARG_DSS_LONG);
    uuid = tree.get<std::string>(ARG_UUID_LONG);
    numEndpoints = tree.get<int>(ARG_NUMENDPOINTS_LONG);
...
void ProgArgs::getAsPropertyTreeForService(bpt::ptree& outTree, size_t serviceRank) const
{
    ...
    outTree.put(ARG_DSS_LONG, useDSS);
    outTree.put(ARG_UUID_LONG, uuid);
    outTree.put(ARG_NUMENDPOINTS_LONG, numEndpoints);
