...
#define ARG_DSS_LONG                "dss"
#define ARG_UUID_LONG               "uuid"
#define ARG_NUMENDPOINTS_LONG       "numendpoints"
#define ARG_NUMENDPOINTS_SHORT      "e"
...
class ProgArgs
{
	...
	private:
		...
		bool useDSS; // use DSS Client library for GET, PUT, DEL only
        std::string uuid; // For user-defined uuid to give to DSS client
        int numEndpoints; // How many endpoints per cluster to use for DSS client
	...
	public:
        ...
        bool getUseDSS() const { return useDSS; }
        std::string getUUID() const { return uuid; }
        int getNumEndpoints() const { return numEndpoints; }
