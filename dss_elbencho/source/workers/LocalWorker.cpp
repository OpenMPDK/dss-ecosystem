...
void LocalWorker::initS3Client()
{
    ...
    if(progArgs->getUseDSS())
        // Assign dss_client from the dss client created in WorkerManager
        dss_client = workersSharedData->dss_client;
...
void LocalWorker::s3ModeUploadObjectSinglePart(std::string bucketName, std::string objectName)
{
    ...
    if(progArgs->getUseDSS())
    {
        // PutObjectBuffer doesn't return bytes read, so just using blocksize for now
        PutObjectBuffer(dss_client, (void*) objectName.c_str(), objectName.length(), (unsigned char*) ioBufVec[0], blockSize);
	...
	const bool isRWMixedReader)
{
	...
	while(rwOffsetGen->getNumBytesLeftToSubmit() )
	{
        ...
        if(progArgs->getUseDSS())
        {
            int ret = -1;
            // ret = content length
            ret = GetObjectBuffer(dss_client, (void*)objectName.c_str(), objectName.length(), (unsigned char*)ioBuf, blockSize);

            // Add bytes to elbencho's tracking variable
            if(isRWMixedReader)
			    atomicLiveOpsReadMix.numBytesDone += ret;
		    else
			    atomicLiveOps.numBytesDone += ret;
        }
        // Using elbencho built-in S3
        else
        {
...
void LocalWorker::s3ModeDeleteObject(std::string bucketName, std::string objectName)
{
	...
	request.WithBucket(bucketName)
		.WithKey(objectName);
    ret = DeleteObject(dss_client, (void *) objectName.c_str(), objectName.length());
    std::cout << "DSS Delete ret: " << ret << std::endl;
