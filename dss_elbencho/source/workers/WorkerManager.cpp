...
void WorkerManager::prepareThreads()
{
    ...
    {
        //Get s3 key
        std::string access = progArgs.getS3AccessKey();
        //Get s3 secret
        std::string secret = progArgs.getS3AccessSecret();
        //Get and parse s3 endpoints
        std::string endpoints_url = progArgs.getS3EndpointsStr();
        std::string endpoint = endpoints_url.substr(endpoints_url.find("//") + 2, endpoints_url.length());
        // Assign uuid
        // For UUID generation 
        std::string uuid_str;
        if(progArgs.getUUID().compare(""))
        {
            uuid_str = progArgs.getUUID();
        }
        else {
        ...
        //Init DSS Client
        workersSharedData.dss_client = (DSSClient*) DSSClientInit((char*) endpoint.c_str(), (char*) access.c_str(), (char*) secret.c_str(), (char *) uuid_str.c_str(), progArgs.getNumEndpoints());
        if(workersSharedData.dss_client == NULL)
        {
            std::cout << "Exception in DSSClientInit while preparing worker threads." << std::endl;
            throw WorkerException("Exception in DSSClientInit.");
        }
